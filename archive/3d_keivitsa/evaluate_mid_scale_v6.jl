#!/usr/bin/env julia
#=
Evaluate mid_scale_prior_v6.jld2 on the COMMEMI 2-D benchmark.

v6 = v5 continued from epoch 35 to 60 (same 400 pairs, same split, cosine
LR picked up at lr_min). Tests whether extra epochs beat the homogeneous
start (best 5.7354) after v5 landed at 11.85.

Usage (from project root):
    julia --project=. scripts/evaluate_mid_scale_v6.jl

Colab (cwd is /content — never `--project=.` from there):
    julia --project=/content/PriorModel /content/PriorModel/scripts/evaluate_mid_scale_v6.jl
...
Steps:
  ...
  3. Print nine-way RMS table + T_ood / x_ood + automatic interpretation
  4. Heatmap: predicted prior vs resampled COMMEMI true model
     → results/evaluate_v6/predicted_vs_true.png
=#
...
function interpret_v6(v6_best)
    if v6_best === nothing
        return "v6 best RMS okunamadı — VFSA çıktısını kontrol edin."
    end
    beats_homo = v6_best < HOMO_BEST
    vs_v5 = v6_best - V5_BEST
    rel_vs_v5 = (V5_BEST - v6_best) / abs(V5_BEST)
    if beats_homo
        return "v6 homojeni (5.7354) geçti (best=$(round(v6_best; digits=4))). " *
               "Geniş pencerede extra epoch işe yaradı; n=1000+ hâlâ düşünülebilir."
    elseif rel_vs_v5 > V4_IMPROVE_REL
        return "v6 homojeni GEÇMEDİ (best=$(round(v6_best; digits=4)) > 5.7354) " *
               "ama v5'e (11.85) göre belirgin iyileşme var (Δ=$(round(vs_v5; digits=4))). " *
               "Daha fazla epoch değil, n=1000+ / çeşitlilik gerekli."
    elseif v6_best < V5_BEST
        return "v6 homojeni GEÇMEDİ (best=$(round(v6_best; digits=4)) > 5.7354). " *
               "v5 (11.85) üzerinde küçük bir kazanç var (Δ=$(round(vs_v5; digits=4))) — " *
               "epoch uzatmak yetmedi, veri/mimari değişmeli."
    else
        return "v6 homojeni GEÇMEDİ (best=$(round(v6_best; digits=4)) > 5.7354) " *
               "ve v5'i (11.85) de geçemedi (Δ=$(round(vs_v5; digits=4))). " *
               "Extra 25 epoch COMMEMI'ye yaramadı."
    end
end

function print_nine_way_table(v6_init, v6_best)
    println("─" ^ 88)
    ...
    println("─" ^ 88)
    println("  v4/v5/v6 mesh: T∈[1e-3,1e3] s, dx=160 m, x∈[-9520,9040] m")
    if v6_best !== nothing
        vs_homo = v6_best - HOMO_BEST
        vs_v5   = v6_best - V5_BEST
        println()
        @printf("  v6 vs homojen (5.7354):  Δbest=%+.4f   %s\n",
                vs_homo, v6_best < HOMO_BEST ? "HOMOJENİ GEÇTİ" : "homojeni GEÇMEDİ")
        @printf("  v6 vs v5     (11.8539):  Δbest=%+.4f   %s\n",
                vs_v5, v6_best < V5_BEST ? "v5'ten iyi" : "v5'ten kötü/eşit")
    end
end
...
function main()
    checkpoint   = joinpath(ROOT, "models", "mid_scale_prior_v6.jld2")
    vfsa_data    = joinpath(ROOT, "examples", "0COMEMI2D-I", "Comemi2D1.obs")
    true_model   = joinpath(ROOT, "examples", "0COMEMI2D-I", "Comemi2D1.true")
    results_dir  = joinpath(ROOT, "results", "evaluate_v6")
    mkpath(results_dir)

    isfile(checkpoint) || error("v6 checkpoint not found: $checkpoint")
    isfile(vfsa_data)  || error("COMMEMI obs not found: $vfsa_data")
    isfile(true_model) || error("COMMEMI true model not found: $true_model")

    println("═" ^ 88)
    println(" Mid-scale v6 Evaluation: U-Net (400/60ep, v5 resume, gT+x, base=32, n_down=3)")
    println("═" ^ 88)

    # ── Step 1: COMMEMI MT → prior .ini ──────────────────────────────────────
    println("\n[1/4] Generating COMMEMI prior from mid_scale_prior_v6.jld2...")
    ...
    prior_ini = joinpath(results_dir, "mid_scale_prior_v6.ini")
    generate_prior(mt_h5, checkpoint, prior_ini;
                   title="U-Net prior v6 (400 samples, 60 ep, resume from v5, wide T+x, COMMEMI MT)")
    println("  → $prior_ini")
    ...
    v6_init = extract_initial_rms(run_dir)
    v6_best = extract_best_rms(run_dir)

    # ── Step 3: Nine-way table + interpretation ──────────────────────────────
    println("\n[3/4] Nine-way RMS comparison")
    print_nine_way_table(v6_init, v6_best)
    interpretation = interpret_v6(v6_best)
    println()
    ...
    open(table_path, "w") do io
        println(io, "Nine-way COMMEMI RMS comparison (1 chain, 200 ctrl, 100 iter, seed=20260308)")
        println(io, "v6 checkpoint: models/mid_scale_prior_v6.jld2")
        ...
        println(io, "")
        if v6_best !== nothing
            @printf(io, "v6 vs homojen (5.7354):  Δbest=%+.4f   %s\n",
                    v6_best - HOMO_BEST,
                    v6_best < HOMO_BEST ? "HOMOJENİ GEÇTİ" : "homojeni GEÇMEDİ")
            @printf(io, "v6 vs v5     (11.8539):  Δbest=%+.4f   %s\n",
                    v6_best - V5_BEST,
                    v6_best < V5_BEST ? "v5'ten iyi" : "v5'ten kötü/eşit")
            println(io, "")
        end
        println(io, "v4/v5/v6 mesh: T∈[1e-3,1e3] s, dx=160 m")
