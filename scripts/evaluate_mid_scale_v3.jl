#!/usr/bin/env julia
#=
Evaluate mid_scale_prior_v3.jld2 on the COMMEMI 2-D benchmark.

v3 is trained on the wide field-MT period band T ∈ [10⁻³, 10³] s so that
COMMEMI T ∈ [0.01, 100] s is in-distribution (the v2 "homojen-beating" result
may have been a period-clamp artefact).

Usage (from project root):
    julia --project=. scripts/evaluate_mid_scale_v3.jl

Colab (cwd is /content — never `--project=.` from there):
    julia --project=/content/PriorModel /content/PriorModel/scripts/evaluate_mid_scale_v3.jl
...
Steps:
  ...
  3. Print six-way RMS table + T_ood status + automatic interpretation
  4. Heatmap: predicted prior vs resampled COMMEMI true model
     → results/evaluate_v3/predicted_vs_true.png
=#
...
# v3 vs v2: relative gap beyond this is "belirgin kötü" (clamp artefact)
const V3_CLOSE_REL = 0.15
...
function interpret_v3(v3_best)
    if v3_best === nothing
        return "v3 best RMS okunamadı — VFSA çıktısını kontrol edin."
    end
    rel = (v3_best - V2_BEST) / abs(V2_BEST)
    if rel <= V3_CLOSE_REL
        return "OOD sorunu v2 sonucunu bozmuyormuş, gerçek öğrenme doğrulandı, production'a güvenle geçilebilir"
    else
        return "v2'nin başarısı kısmen clamp artefaktıydı — v3 (doğru periyot aralığıyla) daha gerçekçi ama daha zayıf bir sonuç veriyor, bu asıl referans alınmalı, n=1000 ölçeği bu temel üzerinden planlanmalı"
    end
end

function print_six_way_table(v3_init, v3_best)
    println("─" ^ 78)
    @printf("  %-44s  initial=%-12s  best=%-12s\n",
    ...
    @printf("  %-44s  initial=%-12s  best=%-12s\n",
            "U-Net (180/25ep, v2, dar periyot):", fmt(V2_INIT), fmt(V2_BEST))
    @printf("  %-44s  initial=%-12s  best=%-12s\n",
            "U-Net (180/25ep, v3, geniş periyot):", fmt(v3_init), fmt(v3_best))
    @printf("  %-44s  initial=%-12s  best=%-12s\n",
            "Gerçek model (sanity):", fmt(TRUE_INIT), fmt(TRUE_BEST))
    println("─" ^ 78)
    println("  v2 = dar T∈[1e-4, 0.1] s; v3 = geniş T∈[1e-3, 1e3] s (aynı split/LR/clip)")
end
...
function main()
    checkpoint   = joinpath(ROOT, "models", "mid_scale_prior_v3.jld2")
    vfsa_data    = joinpath(ROOT, "examples", "0COMEMI2D-I", "Comemi2D1.obs")
    true_model   = joinpath(ROOT, "examples", "0COMEMI2D-I", "Comemi2D1.true")
    results_dir  = joinpath(ROOT, "results", "evaluate_v3")
    mkpath(results_dir)

    isfile(checkpoint) || error("v3 checkpoint not found: $checkpoint")
    isfile(vfsa_data)  || error("COMMEMI obs not found: $vfsa_data")
    isfile(true_model) || error("COMMEMI true model not found: $true_model")

    println("═" ^ 78)
    println(" Mid-scale v3 Evaluation: U-Net (180/25ep, T∈[1e-3, 1e3] s)")
    println("═" ^ 78)

    # ── Step 1: COMMEMI MT → prior .ini ──────────────────────────────────────
    println("\n[1/4] Generating COMMEMI prior from mid_scale_prior_v3.jld2...")
    ...
    prior_ini = joinpath(results_dir, "mid_scale_prior_v3.ini")
    generate_prior(mt_h5, checkpoint, prior_ini;
                   title="U-Net prior v3 (180 samples, 25 ep, wide periods, COMMEMI MT)")
    println("  → $prior_ini")
    ...
    v3_init = extract_initial_rms(run_dir)
    v3_best = extract_best_rms(run_dir)

    # ── Step 3: Six-way table + interpretation ───────────────────────────────
    println("\n[3/4] Six-way RMS comparison")
    print_six_way_table(v3_init, v3_best)
    interpretation = interpret_v3(v3_best)
    println()
    ...
    open(table_path, "w") do io
        println(io, "Six-way COMMEMI RMS comparison (1 chain, 200 ctrl, 100 iter, seed=20260308)")
        println(io, "v3 checkpoint: models/mid_scale_prior_v3.jld2")
        println(io, "VFSA run_dir:  $run_dir")
        println(io, "T_ood (COMMEMI ⊂ eğitim T): $T_ood")
        println(io, "")
        @printf(io, "  %-44s  initial=%-12s  best=%-12s\n",
                "Homojen:", fmt(HOMO_INIT), fmt(HOMO_BEST))
        @printf(io, "  %-44s  initial=%-12s  best=%-12s\n",
                "U-Net (20/10ep):", fmt(UNET20_INIT), fmt(UNET20_BEST))
        @printf(io, "  %-44s  initial=%-12s  best=%-12s\n",
        ...
        @printf(io, "  %-44s  initial=%-12s  best=%-12s\n",
                "Gerçek model (sanity):", fmt(TRUE_INIT), fmt(TRUE_BEST))
        println(io, "")
        println(io, "v2 = dar T∈[1e-4, 0.1] s; v3 = geniş T∈[1e-3, 1e3] s")
