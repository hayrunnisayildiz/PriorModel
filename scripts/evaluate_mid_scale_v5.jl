#!/usr/bin/env julia
#=
Evaluate mid_scale_prior_v5.jld2 on the COMMEMI 2-D benchmark.

v5 = wide T + wide x (dx=160 m, 19.2 km) with higher U-Net capacity
(base_channels=32, n_down=3) and n=400 / 35 epochs. Addresses the v4
failure (best RMS 18.86 vs homogeneous 5.74) by growing both data and
capacity together.

Usage (from project root):
    julia --project=. scripts/evaluate_mid_scale_v5.jl

Colab (cwd is /content — never `--project=.` from there):
    julia --project=/content/PriorModel /content/PriorModel/scripts/evaluate_mid_scale_v5.jl
...
Steps:
  ...
  3. Print eight-way RMS table + T_ood / x_ood + automatic interpretation
  4. Heatmap: predicted prior vs resampled COMMEMI true model
     → results/evaluate_v5/predicted_vs_true.png
=#
...
const V4_IMPROVE_REL = 0.15   # v5 must beat v4 by more than this to count as "belirgin"
...
function interpret_v5(v5_best)
    if v5_best === nothing
        return "v5 best RMS okunamadı — VFSA çıktısını kontrol edin."
    end
    beats_homo = v5_best < HOMO_BEST
    rel_vs_v4 = (V4_BEST - v5_best) / abs(V4_BEST)
    better_than_v4 = rel_vs_v4 > V4_IMPROVE_REL
    if beats_homo
        return "Geniş pencerede de çalışıyor, production'a (n=1000+) güvenle geçilebilir"
    elseif better_than_v4
        return "Doğru yönde, n=1000+ ölçeği gerekli, kapasite artışı da işe yaradı"
    else
        return "Kapasite artışı tek başına yetmiyor, 400 örnek de yetersiz — mimari YA DA veri çeşitliliği stratejisi (senaryo sayısını 8'den artırma) yeniden düşünülmeli"
    end
end

function print_eight_way_table(v5_init, v5_best)
    println("─" ^ 88)
    ...
    println("─" ^ 88)
    println("  v4/v5 mesh: T∈[1e-3,1e3] s, dx=160 m, x∈[-9520,9040] m")
end
...
function main()
    checkpoint   = joinpath(ROOT, "models", "mid_scale_prior_v5.jld2")
    vfsa_data    = joinpath(ROOT, "examples", "0COMEMI2D-I", "Comemi2D1.obs")
    true_model   = joinpath(ROOT, "examples", "0COMEMI2D-I", "Comemi2D1.true")
    results_dir  = joinpath(ROOT, "results", "evaluate_v5")
    mkpath(results_dir)

    isfile(checkpoint) || error("v5 checkpoint not found: $checkpoint")
    isfile(vfsa_data)  || error("COMMEMI obs not found: $vfsa_data")
    isfile(true_model) || error("COMMEMI true model not found: $true_model")

    println("═" ^ 88)
    println(" Mid-scale v5 Evaluation: U-Net (400/35ep, gT+x, base=32, n_down=3)")
    println("═" ^ 88)

    # ── Step 1: COMMEMI MT → prior .ini ──────────────────────────────────────
    println("\n[1/4] Generating COMMEMI prior from mid_scale_prior_v5.jld2...")
    ...
    prior_ini = joinpath(results_dir, "mid_scale_prior_v5.ini")
    generate_prior(mt_h5, checkpoint, prior_ini;
                   title="U-Net prior v5 (400 samples, 35 ep, wide T+x, large capacity, COMMEMI MT)")
    println("  → $prior_ini")
    ...
    v5_init = extract_initial_rms(run_dir)
    v5_best = extract_best_rms(run_dir)

    # ── Step 3: Eight-way table + interpretation ─────────────────────────────
    println("\n[3/4] Eight-way RMS comparison")
    print_eight_way_table(v5_init, v5_best)
    interpretation = interpret_v5(v5_best)
    println()
    ...
    open(table_path, "w") do io
        println(io, "Eight-way COMMEMI RMS comparison (1 chain, 200 ctrl, 100 iter, seed=20260308)")
        println(io, "v5 checkpoint: models/mid_scale_prior_v5.jld2")
        ...
        @printf(io, "  %-58s  initial=%-12s  best=%-12s\n",
                "Gerçek model (sanity):", fmt(TRUE_INIT), fmt(TRUE_BEST))
        println(io, "")
        println(io, "v4/v5 mesh: T∈[1e-3,1e3] s, dx=160 m")
